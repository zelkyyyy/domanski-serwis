...
const Services = ({ services }: ServicesProps) => {
  ...
  return (
    <div
      id="usługi"
      className="relative flex w-full flex-col items-center justify-center my-24 min-h-[500px]"
    >
      <h2 className="mb-16 text-5xl font-bold tracking-wide text-neutral-200 font-sans">
        Nasze Usługi
      </h2>
          ...
          return (
            ...
            >
              <button
                ...
                className="group flex w-full items-center justify-between gap-4 px-5 py-4 text-left transition-colors hover:bg-white/5"
                aria-expanded={isOpen}
                aria-controls={`service-panel-${index}`}
              >
                <span className="text-lg font-semibold text-neutral-100 md:text-xl">
                  {service.name}
                </span>
                <div
                  className={`w-0 h-px bg-red-500 transition-transform duration-300 `}
                ></div>
                <span
                  className="flex h-10 w-10 items-center justify-center rounded-full transition-colors group-hover:bg-white/10"
                  aria-hidden="true"
                >
                  <IoAdd
                    className={`h-5 w-5 text-red-500 transition-transform duration-300 ${isOpen ? "rotate-45" : "rotate-0"}`}
                  />
                </span>
              </button>
